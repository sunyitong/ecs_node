...
use crate::core::display_style::{LINE_STYLE_TEST, TEXT_STYLE_LARGE, TEXT_STYLE_SMALL};
use crate::core::display_trait::DisplayDraw;
use crate::platform::platform_data::{TILE_MAP_DIMENSION, TILE_MAP_MARGIN, TILE_NODE_DIMENSION};

pub fn spawn_test_node(mut commands: Commands){
    commands.spawn((
        NodeName("Test Node_1".to_string()),
        NodeTilePosition((0,0)),
        NodeInputs(vec![0.123, 0.324, 0.221, 0.232, 0.1, 0.99]),
        NodeOutputs(vec![0.114555, 0.1881]),
    ));

    commands.spawn((
        NodeName("Test Node_2".to_string()),
        NodeTilePosition((0,1)),
        NodeInputs(vec![0.123, 0.324, 0.221, 0.232, 0.1, 0.99]),
        NodeOutputs(vec![0.0,]),
    ));
}
...
pub fn draw_test_node(
    ...
    for (name, position, inputs, outputs) in query_test_node.iter() {
        ...
        for (c,v) in inputs.0.iter().enumerate(){
            display.draw_text(&*v.to_string(), node_ref_position_x, node_ref_position_y + input_spacing as i32 *(c as i32+1), TEXT_STYLE_SMALL);
        }
        for (c ,v) in outputs.0.iter().enumerate(){
            display.draw_text_right(&*v.to_string(), node_ref_position_x+TILE_NODE_DIMENSION as i32, node_ref_position_y + output_spacing as i32 *(c as i32+1), TEXT_STYLE_SMALL);
        }
    }
}